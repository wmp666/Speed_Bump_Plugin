 ...
 * {@code false}，相关链接方法均为空实现，插件实际行为（检测前台窗口标题 + 显示遮挡层）由
 * {@link MaskWatcher} 承担。</p>
 *
 * @author 无名牌
 */
public class WindowMask extends AbstractParser {

    static {
        // 插件主类被主程序加载（{@code Class.forName(..., true, loader)}）时，检测线程即开始工作。
        MaskWatcher.getInstance().start();
    }
